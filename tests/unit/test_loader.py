...
from tests.base import RAML_08, JSONREF
...
def test_load_file():
    raml_file = os.path.join(RAML_08, "base-includes.raml")
...
def test_load_file_with_nested_includes():
    raml_file = os.path.join(RAML_08, "nested-includes.raml")
    with open(raml_file) as f:
        raml = loader.RAMLLoader().load(f)
...
def test_load_file_with_nonyaml_include():
    raml_file = os.path.join(RAML_08, "nonyaml-includes.raml")
    with open(raml_file) as f:
        raml = loader.RAMLLoader().load(f)
...
def test_yaml_parser_error():
    raml_obj = os.path.join(RAML_08, "invalid_yaml.yaml")
...
def test_include_json():
    raml_file = os.path.join(RAML_08, "json_includes.raml")
    with open(raml_file) as f:
        raml = loader.RAMLLoader().load(f)
...
def test_include_xsd():
    raml_file = os.path.join(RAML_08, "xsd_includes.raml")
    with open(raml_file) as f:
        raml = loader.RAMLLoader().load(f)
...
def test_include_markdown():
    raml_file = os.path.join(RAML_08, "md_includes.raml")
    with open(raml_file) as f:
        raml = loader.RAMLLoader().load(f)
...
def test_invalid_yaml_tag():
    raml_file = os.path.join(RAML_08, "invalid_yaml_tag.raml")
    with pytest.raises(LoadRAMLError) as e:
...
def test_includes_has_invalid_tag():
    raml_file = os.path.join(RAML_08, "include_has_invalid_tag.raml")
    with pytest.raises(LoadRAMLError) as e:
        loader.RAMLLoader().load(open(raml_file))
